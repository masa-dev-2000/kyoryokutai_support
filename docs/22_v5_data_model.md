...
| source_municipality_id | uuid |  | 元自治体 ID(管理者のみ参照可能) |
| title | text | ✓ | |
| area_label | text | ✓ | 例:「兵庫県 西部」 |
| year | text | ✓ | 例:"2024" |
| author_label | text | ✓ | 例:「移住促進担当」 |
| summary | text | ✓ | |
| kpi | text |  | |
| effect | text |  | |
| process | jsonb |  | プロセス配列 |
| learning | text |  | 学び |
...
**RLS:**
- 全テナントから SELECT 可能(`is_public_case = true`)
- INSERT は Edge Function のみ(匿名化処理経由)
