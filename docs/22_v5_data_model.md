...
| # | サブシステム | 責務 | 主要テーブル |
|---|---|---|---|
| **A** | **Identity & Tenant** | 自治体・ユーザー・担当割当(全サブシステムの基盤) | `municipalities` / `users` / `assignments` |
| **B** | **Activity Recording** | 活動内容テンプレ・日次記録・プロジェクト管理 | `activity_topics` / `activity_logs` / `projects` |
| **C** | **Reporting** | 月次報告(AI 生成 + 提出) | `monthly_reports` |
| **D** | **Expense** | 経費の申請 / 精算 | `expenses` |
| **E** | **Workflow & Approval** | 汎用承認ワークフロー(経費・月報・活動相談を統一) | `approvals` |
| **F** | **Communication** | お知らせ配信と既読管理 | `announcements` / `announcement_reads` |
...
**説明:** マルチテナント分離の基盤。`users.role` で `member`(隊員)/ `manager`(役場職員)/ `admin`(管理者)を識別。`assignments` は職員 × 隊員の N:N 担当割当で、RLS の「管轄判定」に使われる。
...
### 3.E Workflow & Approval(汎用ワークフロー)

```mermaid
erDiagram
  users ||--o{ approvals : "applicant(起票)"
...
- 役場側「承認」タブの統一データ源。**差戻し時のみ `comment` 必須**(v3 仕様)。
...
```mermaid
erDiagram
  users ||--o{ announcements : "sender(役場)"
  announcements ||--o{ announcement_reads : "既読記録"
  users ||--o{ announcement_reads : "閲覧者"
```

**説明:** お知らせは `target_user_ids` 配列で宛先を保持(全体送信 = 自治体内隊員全 ID)。`announcement_reads` で既読率を測る。
...
erDiagram
  ...
  users ||--o{ approvals : "(applicant / approver)"
