...
type Sheet =
  ...
  | { kind: "expense-create"; date?: string; dailyLogId?: string }
...
type Ctx = {
  ...
  updateDailyLog: (date: string, fields: Partial<Pick<DailyLogEntry, "distanceKm" | "feelingScore" | "expenseAmount">>) => Promise<void>;
  topics: string[];
  ...
  addExpense: (e: Omit<ExpenseRequest, "id" | "createdAt" | "aiNote" | "citation" | "hasReceipt"> & { date?: string }) => void | Promise<void>;
...
export function MemberApp() {
  ...
  const ctx: Ctx = {
    ...
    updateDailyLog: async (date, fields) => {
      const updated = await apiPost<DailyLogEntry>("/api/daily-logs", { userId: memberId, date, activities: [], ...fields });
      setDailyLogs((dls) => [updated, ...dls.filter((d) => d.date !== updated.date)].sort((a, b) => b.date.localeCompare(a.date)));
    },
    addLog: async (l) => {
    ...
    addExpense: async (e) => {
      ...
      try {
        const ds = await apiGet<DailyLogEntry[]>(`/api/daily-logs?userId=${memberId}`);
        setDailyLogs(ds);
      } catch { /* noop */ }
    },
