...
function ActivityCreateSheet({ onClose, editing, date }: { onClose: () => void; editing?: ActivityLog; date?: string }) {
  ...
  // 音声入力をメモ末尾に追記
  function appendActivityBody(idx: number, text: string) {
    setActivities((cur) => cur.map((a, i) => (i === idx ? { ...a, body: a.body ? `${a.body} ${text}` : text } : a)));
  }
  // AI 質問補完(P0-2): メモから「次の一問」を取得して表示
  const [followupQ, setFollowupQ] = React.useState<Record<number, string>>({});
  const [loadingQ, setLoadingQ] = React.useState<number | null>(null);
  async function askFollowup(idx: number, current: string) {
    setLoadingQ(idx);
    try {
      const r = await apiPost<{ question: string }>("/api/ai/followup", { current });
      if (r.question) setFollowupQ((q) => ({ ...q, [idx]: r.question }));
    } catch {
      /* noop */
    } finally {
      setLoadingQ(null);
    ...
    return (
      <>
        ...
        <div className="mx-auto w-full max-w-2xl flex-1 overflow-y-auto px-6 py-6">
          ...
          <Label right={<VoiceInput onText={(t) => setEditBody((b) => (b ? `${b} ${t}` : t))} />}>メモ</Label>
          ...
          {activities.map((a, i) => (
            <div key={i} className="rounded-xl border border-slate-200 bg-white p-4">
              ...
              <div className="mt-2">
                <Label
                  right={
                    <div className="flex items-center gap-1.5">
                      <VoiceInput onText={(t) => appendActivityBody(i, t)} />
                      <button
                        type="button"
                        onClick={() => askFollowup(i, a.body)}
                        disabled={loadingQ === i}
                ...
                <textarea rows={3} value={a.body} onChange={(e) => updateActivity(i, { body: e.target.value })} placeholder="話す(音声)か入力。例:A 邸を内覧、移住希望者と一緒に。築 80 年だが構造良好。" className="mt-1 w-full resize-none rounded-xl border border-slate-300 bg-white px-3 py-2 text-[17px] focus:border-slate-900 focus:outline-none" />
                {followupQ[i] && (
                  <div className="mt-1 flex items-start gap-1.5 rounded-lg bg-amber-50 px-2.5 py-1.5 text-[13px] text-amber-900">
                    <Lightbulb className="mt-0.5 h-3.5 w-3.5 shrink-0" />
                    <span>{followupQ[i]}</span>
                  </div>
                )}
              </div>
            </div>
